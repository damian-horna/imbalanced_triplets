...
def train_safenessnet(model, device, train_loader, optimizer, epoch, weights, nn_config, log_interval=10, pca=None, X_train=None):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        invert_op = getattr(loss, "backward", None)
...
class SafenessLoss(nn.Module):
    ...
    def forward(self, embeddings, target):
        ...
        for i in range(batch_size):
            ...
            # print(len(same_class_dists))
            # print("-")
            # print(len(different_class_dists))

            if torch.is_tensor(same_class_dist_sum - different_class_dist_min_sum):
                losses.append((same_class_dist_sum/(len(same_class_dists) + 1e-6)
                               - different_class_dist_min_sum / (len(different_class_dists) + 1e-6))
                              * (1 - len(emb_same_class) / len(embeddings[1:])))
        ...
        # print("Losses:")
        # print(losses)
        result = torch.stack(losses).mean() if losses else 0
        return result
...
def init_loaders(X_train, X_test, y_train, y_test, train_repr, test_repr, batch_size, test_batch_size, use_cuda, k_neigh, ds_name):
    ...
    neighbors_train_dataset = NeighborsDataset(dataset1, train_repr, n_neighbors=k_neigh, ds_name=ds_name)
    neighbors_test_dataset = NeighborsDataset(dataset2, test_repr, n_neighbors=k_neigh, ds_name=ds_name)
...
class NeighborsDataset(Dataset):
    def __init__(self, ds, representation, n_neighbors=21, ds_name=None):
        np.random.seed(0)
        self.ds = ds
        ...
        if self.train:
            ...
            self.minority_indices = [idx for idx, label in enumerate(self.ds.train_labels) if label in dt_name_minority_classes[ds_name]]
        else:
            self.test_labels = self.ds.test_labels
            self.test_data = self.ds.test_data
            self.minority_indices = [idx for idx, label in enumerate(self.ds.test_labels) if label in dt_name_minority_classes[ds_name]]
        self.neigh.fit(self.representation)

    def __getitem__(self, index):
        anchor_repr = self.representation[index, :]
        if self.train:
            anchor = self.train_data[self.minority_indices[index]]
            anchor_label = self.train_labels[self.minority_indices[index]]
            neigh_indices = self.neigh.kneighbors([anchor_repr], return_distance=False)
            neigh_indices = [ind for ind in neigh_indices[0] if ind != self.minority_indices[index]] # without self
            ...
            anchor = self.test_data[self.minority_indices[index]]
            anchor_label = self.test_labels[self.minority_indices[index]]
            neigh_indices = self.neigh.kneighbors([anchor_repr], return_distance=False)
            neigh_indices = [ind for ind in neigh_indices[0] if ind != self.minority_indices[index]] # without self
            neigh_indices = neigh_indices[: self.n_neighbors-1]
    ...
    def __len__(self):
        return len(self.minority_indices)
